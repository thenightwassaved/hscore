...
local int count_current_playing(Arena *arena) //fm_normal
{
	Player *p;
	Link *link;
	int playing = 0;
...
local int count_freq(Arena *arena, int freq, Player *excl, int inclspec) //fm_normal
{
	Player *p;
	Link *link;
	int t = 0;
...
local int FindLegalShip(Player *p, int freq, int ship) //fm_normal
{
	Arena *arena = p->arena;
	...
	else
	{
		...
		if (cfg->GetInt(arena->cfg, ship_names[ship], "BuyPrice", 0))
		{
			HSData *data = db->GetHSData(p);

			if (data->hull_status[ship] == HULL_NO_HULL)
			{
				chat->SendMessage(p, "You do not own a %s hull. Please use \"?buy ships\" to examine the ship hulls for sale.", ship_names[ship]);
				return SHIP_SPEC;
			}
			else if (data->hull_status[ship] == HULL_NOT_LOADED)
			{
...
local int BalanceFreqs(Arena *arena, Player *excl, int inclspec) //fm_normal
{
	Player *i;
	Link *link;
...
local int screen_res_allowed(Player *p, ConfigHandle ch) //fm_normal
{
...
local void Initial(Player *p, int *ship, int *freq) //fm_normal
{
	Arena *arena = p->arena;
	int f, s = *ship;
	ConfigHandle ch;
...
local void Ship(Player *p, int *ship, int *freq) //fm_normal
{
	Arena *arena = p->arena;
	int f = *freq, s = *ship;
	ConfigHandle ch;
...
local void Freq(Player *p, int *ship, int *freq) //fm_normal
{
	Arena *arena = p->arena;
	int f = *freq, s = *ship;
...
{
	INTERFACE_HEAD_INIT(I_FREQMAN, "hscore_freqman")
	Initial, Ship, Freq
};

EXPORT int MM_hscore_freqman(int action, Imodman *_mm, Arena *arena)
{
	if (action == MM_LOAD)
	{
		mm = _mm;
		...
		if (!lm || !chat || !cfg || !cmd || !db || !pd || !capman)
		{
